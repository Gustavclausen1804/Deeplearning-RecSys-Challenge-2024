...
class NRMSDataLoader(NewsrecDataLoader):
    ...
    def __getitem__(self, idx) -> tuple[tuple[np.ndarray], np.ndarray]:
        print(f"\nTensorFlow Dataloader - Batch {idx}:")
        ...
        print(f"Initial batch_X shape: {len(batch_X)}")
        print(f"Initial batch_y shape: {len(batch_y)}")

        if self.eval_mode:
            repeats = np.array(batch_X["n_samples"])
            print(f"Eval mode repeats: {repeats}")
            
            batch_y = np.array(batch_y.explode().to_list()).reshape(-1, 1)
            print(f"Processed batch_y shape: {batch_y.shape}")
            ...
            print(f"his_input_title shape after repeat: {his_input_title.shape}")
            ...
            print(f"Training batch_y shape: {batch_y.shape}")
            
            his_input_title = self.lookup_article_matrix[
                batch_X[self.history_column].to_list()
            ]
            print(f"his_input_title initial shape: {his_input_title.shape}")
            
            pred_input_title = self.lookup_article_matrix[
                batch_X[self.inview_col].to_list()
            ]
            print(f"pred_input_title initial shape: {pred_input_title.shape}")
        ...
        print("\nSample values:")
        print(f"his_input_title first element: {his_input_title[0, 0, :5]}")
        print(f"pred_input_title first element: {pred_input_title[0, 0, :5]}")
        print(f"batch_y first element: {batch_y[0]}")
